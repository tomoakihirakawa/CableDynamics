...
節点数を変えて端点張力の収束を確認した結果（斜張橋 C01–C12，全 12 本）:

![収束グラフ — 全 12 本](cable/docs/convergence_all_12cables.png)

- 右側 log-log の「Relative error」は **n=201（最精密離散化）を参照値**とした相対誤差であり，解析解や実測値との比較ではない
- このグラフは「Lumped Mass モデルの**離散化誤差の収束傾向**」を示すものであり，「物理的に正しい値への収束」を保証するものではない
- 全ケーブルが O(1/n) の 1 次収束（Lumped Mass の線形要素として妥当）
- n=31（デフォルト）で離散化誤差 < 0.04%
- 物理的妥当性の検証は，上記「検証」セクションの FEM 比較（最大 RMS 0.56%）で確認済み
...
└── cable/                      # ケーブル動力学ソルバ
    ...
    │   ├── examples/           # サンプル JSON + yuri_bridge/（斜張橋 12 本）
    │   └── tests/              # pytest（43 テスト）
```
